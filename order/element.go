...
// Modulus q =
//
//	q[base10] = 73846995687063900142583536357581573884798075859800097461294096333596429543
//	q[base16] = 0x29cbc14e5e0a72f05397829cbc14e5dfbd004dfe0f79992fb2540ec7768ce7
//
// # Warning
//
...
const (
	...
	Bits  = 246 // number of bits needed to represent a Element
...
const qInvNeg uint64 = 16226440803436476713

func init() {
	_modulus.SetString("29cbc14e5e0a72f05397829cbc14e5dfbd004dfe0f79992fb2540ec7768ce7", 16)
}
...
func (z *Element) SetInterface(i1 interface{}) (*Element, error) {
	if i1 == nil {
		return nil, errors.New("can't set order.Element with <nil>")
	}
	...
	default:
		return nil, errors.New("can't set order.Element from type " + reflect.TypeOf(i1).String())
	}
}
...
func (z *Element) SetOne() *Element {
	z[0] = 15833853072060250400
	z[1] = 11123365838948087227
	z[2] = 5
	z[3] = 0
	return z
}
...
func (z *Element) IsOne() bool {
	return (z[3] | (z[2] ^ 5) | (z[1] ^ 11123365838948087227) | (z[0] ^ 15833853072060250400)) == 0
}
...
func (z *Element) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 10941822980899686004, 0)
	_, b = bits.Sub64(_z[1], 17284393324870352076, b)
	_, b = bits.Sub64(_z[2], 8658675789700401778, b)
	_, b = bits.Sub64(_z[3], 5882252574524729, b)
...
func (z *Element) SetBytesCanonical(e []byte) error {
	if len(e) != Bytes {
		return errors.New("invalid order.Element encoding")
	}
	v, err := BigEndian.Element((*[Bytes]byte)(e))
	if err != nil {
		return err
...
func (bigEndian) Element(b *[Bytes]byte) (Element, error) {
	...
	if !z.smallerThanModulus() {
		return Element{}, errors.New("invalid order.Element encoding")
	}
...
func init() {
	_bLegendreExponentElement, _ = new(big.Int).SetString("14e5e0a72f05397829cbc14e5e0a72efde8026ff07bccc97d92a0763bb4673", 16)
	const sqrtExponentElement = "a72f05397829cbc14e5e0a72f053977ef40137f83de664bec9503b1dda33a"
...
const (
	inversionCorrectionFactorWord0 = 7647898691114033823
	inversionCorrectionFactorWord1 = 17391408356211640492
	inversionCorrectionFactorWord2 = 2878155868934822524
	inversionCorrectionFactorWord3 = 6816266662392934
	invIterationsN                 = 16
)
